...
//Comments add and delete, adds @mentions Evidence
//View attachements
...
module.exports = {
  ...
  'Interview comments edit @mentions': function(browser) {
      browser
        ...
        .keys('The first film, Raiders of the Lost Ark (1981) is set in 1936. Indiana Jones (Harrison Ford) is hired by government agents to locate the Ark of the Covenant before the Nazis. The Nazis have teams searching for religious artifacts, including the Ark, which is rumored to make an army that carries the Ark before it invincible.')
        .keys(browser.Keys.ENTER)
        .click("(//div[@class='comment-editor'])[3]").pause(model.pause + 800)
        .keys('@')
        .keys(browser.Keys.ENTER)
        ...
        .keys(browser.Keys.ENTER)
        .pause(model.pause + 1000)
        //.verify.elementPresent("(//div[@class='comment'])", 'edit comments').pause(model.pause + 2000)
 ...
 'View connections in ideas': function(browser) {
      browser
        ...
        .pause(model.pause + 800)
        .verify.elementPresent("(//div[@class='card-content'])[1]", 'edits a collection').pause(model.pause + 800)
  ...
  'View activity feed': function(browser) {
      browser
        .waitForElementPresent("(//div[@class='basic-dropdown-selected-text center-nav-page-routes '])", 1000, 'opens for feed')
